    ...
    <form id="create-artist-form">
        ...
        <div id="albums-container">
            <div class="album-entry">
                ...
                <input type="date" class="album-release-date">
                <label for="albumTracks">Tracks (comma-separated):</label>
                <input type="text" class="album-tracks">
                
                <label>Album Cover:</label>
                <div>
                    <input type="radio" id="albumUploadImage" name="albumImageSource" value="upload" checked>
                    <label for="albumUploadImage">Upload</label>
                    <input type="radio" id="albumImageLink" name="albumImageSource" value="link">
                    <label for="albumImageLink">Link</label>
                </div>
                <input type="file" class="album-image-upload" accept="image/*">
                <input type="text" class="album-image-link" placeholder="Image Link" style="display: none;">
            </div>
        </div>
...
async function handleCreateArtistFormSubmission(event) {
  ...
  newArtist.genres.forEach((genre) => formData.append("genres", genre));
  formData.append("biography", newArtist.biography);

  const uploadImageRadio = document.getElementById("uploadImage");
  const imageFileElement = document.getElementById("imageUpload");
  if (uploadImageRadio.checked && imageFileElement) {
...
// Define albumElements here
  ...
  const albums = Array.from(albumElements).map((albumElement) => {
    const album = {
        ...
        releaseDate: albumElement.querySelector(".album-release-date").value,
        tracks: albumElement.querySelector(".album-tracks").value.split(",").map((track) => track.trim()),
    };

    const albumUploadImageRadio = albumElement.querySelector("#albumUploadImage");
    const albumImageFileElement = albumElement.querySelector(".album-image-upload");
    const albumImageLinkInput = albumElement.querySelector(".album-image-link");

    if (albumUploadImageRadio.checked && albumImageFileElement.files.length > 0) {
        album.image = albumImageFileElement.files[0];
    } else if (albumImageLinkInput.value.trim() !== "") {
        album.imageLink = albumImageLinkInput.value.trim();
    }
  ...
  try {
    ...
    if (album.image) {
        ...
        imageData.append("image", album.image);
    } else if (album.imageLink) {
        imageData = album.imageLink;
    }

    const createdAlbum = await createAlbum(album.name, artistData.id, null, imageData);
  }
    } else {
...
export function showEditArtistModal(artist) {
    ...
    const formHTML = `
        <form id="edit-artist-form">
            <img src="http://localhost:3006/artists/${artist.id}/image" id="EditArtistImage" alt="Edit Artist"/>
            <label for="name">Name:</label>
            <input type="text" id="name" required>
            <label>Genres:</label>
            <div id="genres-bobbles">
