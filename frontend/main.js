...
document
  ...
  .addEventListener("click", navigateToArtistsPage);
document
  .getElementById("albumsPage")
  .addEventListener("click", navigateToAlbumsPage);
document
  .getElementById("tracksPage")
  .addEventListener("click", navigateToTracksPage);
document.getElementById("genrePage").addEventListener("click", showGenreModal);
document
  .getElementById("favoritesPage")
  .addEventListener("click", navigateToFavoritesPage);
document
  .getElementById("aboutPage")
  .addEventListener("click", navigateToAboutPage);
...
async function searchAndUpdateArtists(query) {
  try {
    const response = await fetch(
      `http://localhost:3006/artists/search?q=${query}`
    );
...
document
  .querySelector(".close-btn-create")
...
function getUniqueGenres() {
  ...
  artists.forEach((artist) => {
    if (artist.genres) {
      // Check if artist.genres is defined
...
function navigateToGenrePage(event) {
  ...
  uniqueGenres.forEach((genre, index) => {
    ...
    if ((index + 1) % 5 === 0 || index === uniqueGenres.length - 1)
      genreListHTML += `</div>`;
  });
...
function toggleFavorite(artistId) {
  ...
  const heartButton = document.querySelector(
    `.favorite-btn[data-artist-id="${artistId}"]`
  );
  if (heartButton) {
  ...
  if (
    ...
    sessionStorage.getItem("lastTab") === "favorites"
  ) {
  }
}
